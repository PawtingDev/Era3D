...
from mvdiffusion.data.thuman_dataset import THumanDataset
...
def main(
    ...
    # Freeze camera regressor in unet
    if cfg.untrainable_modules is None:
        unet.requires_grad_(True)
    else:
        unet.requires_grad_(True)
        for name, module in unet.named_modules():
            if name.endswith(tuple(cfg.untrainable_modules)):
                for params in module.parameters():
                    params.requires_grad = False
                    # print("untrainable: ", params, params.shape)
    ...
    train_dataset = THumanDataset(
        validation=False, **cfg.train_dataset
    )
    validation_dataset = THumanDataset(
        validation=True, **cfg.validation_dataset
    )
    ...
    image_normalizer.to(accelerator.device)
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with (accelerator.accumulate(unet)):
                # (B, Nv, 3, H, W)
                imgs_in, colors_out, normals_out = batch['imgs_in'], batch['imgs_out'], batch['normals_out']
                normal_text_embeds, color_text_embeds = batch['normal_text_embeds'], batch['color_text_embeds']
                bnm, Nv = imgs_in.shape[:2]

                # repeat  (2B, Nv, 3, H, W)
                imgs_in = torch.cat([imgs_in] * 2, dim=0)
                imgs_out = torch.cat([normals_out, colors_out], dim=0)
                prompt_embeddings = torch.cat([normal_text_embeds, color_text_embeds], dim=0)

                # (B*Nv, 3, H, W)
                imgs_in = rearrange(imgs_in, "B Nv C H W -> (B Nv) C H W").to(weight_dtype)
                imgs_out = rearrange(imgs_out,"B Nv C H W -> (B Nv) C H W").to(weight_dtype)
                prompt_embeddings = rearrange(prompt_embeddings, "B Nv N C -> (B Nv) N C").to(weight_dtype)
                ...
                # same condition for diffrent views
                # (B*Nv, 1024*2)
                image_embeds = image_embeds.repeat(1, 1)
                ...
                if cfg.use_classifier_free_guidance and cfg.condition_drop_rate > 0.:
                    ...
                    if cfg.drop_type == 'drop_as_a_whole':
                        ...
                        # Sample masks for the conditioning text.
                        clip_mask_dtype = prompt_embeddings.dtype
                        clip_mask = 1 - (
                        ...
                        # Final image conditioning.
                        prompt_embeddings = clip_mask * prompt_embeddings
